...
public class ChannelUpdateHandler extends SocketHandler {
    ...
    protected Long handleInternally(DataObject content) {
        ...
        boolean wasObfuscated = channel.isObfuscated();
        ...
        if (channel instanceof ITopicChannelMixin<?> topicChannel) {
            dispatch.handleTopic(topicChannel, content.getString("topic", null));
        }

        if (channel instanceof ISlowmodeChannelMixin<?> slowmodeChannel) {
            dispatch.handleSlowmode(slowmodeChannel, content.getInt("rate_limit_per_user", 0));
        }

        if (channel instanceof IAgeRestrictedChannelMixin<?> ageRestrictedChannel) {
            dispatch.handleNsfw(ageRestrictedChannel, content.getBoolean("nsfw"));
        }

        if (channel instanceof ICategorizableChannelMixin<?> categorizableChannel) {
            dispatch.handleParentCategory(categorizableChannel, content.getUnsignedLong("parent_id", 0));
        }

        if (channel instanceof IPositionableChannelMixin<?> positionableChannel) {
            dispatch.handlePosition(positionableChannel, content.getInt("position", 0));
        }

        if (channel instanceof IThreadContainerMixin<?> threadContainer) {
            dispatch.handleThreadContainer(threadContainer, content);
        }

        if (channel instanceof AudioChannelMixin<?> audioChannel) {
            dispatch.handleAudioChannel(audioChannel, content);
        }

        if (channel instanceof IPostContainerMixin<?> postContainer) {
            dispatch.handlePostContainer(postContainer, content);
        }
    ...
    private AbstractGuildChannelImpl<?> handleChannelTypeChange(
        ...
        getJDA().handleEvent(new ChannelUpdateTypeEvent(getJDA(), responseNumber, newChannel, oldType, newChannelType));
